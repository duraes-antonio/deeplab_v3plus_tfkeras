...
from utils import build_config_with_paths, get_model_file_name
...
model_file = os.path.join(model_dir, get_model_file_name(conf))
...
if "test" in which_to_inference:
	...
	dataset, path_list = make_dataset(
		test_x_dirs, image_size, label, preprocess, batch_size,
		y_dirs=test_y_dirs, extra_x_dirs=test_extra_x_dirs,
		n_extra_channels=n_extra_channels, data_augment=False,
		shuffle=False
	)
	pred = model.predict(dataset, verbose=1)
	fpath = os.path.join(model_dir, "test_inference.h5")
	save_inference_results(fpath, pred, last_activation)
